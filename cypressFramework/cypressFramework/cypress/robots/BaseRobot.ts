...
//costum
  ...
  seesTextWithDomPath(domlabel: string, matcher: string, grandParent: string, parent: string, child:string, text: string) {
    cy.get(`[${domlabel}=${matcher}]`).find(grandParent).find(parent).find(child).should('contain', text).debug();
...
//
  ...
  seesAttributeValue(dom: string, attribute: string, data: string) {
    cy.get(dom).invoke('attr',attribute).should('equal', data )
  }
}
...
export class BaseHands {
  ...
  selectElementFromAutoSuggestions(locatorName: string, locatorValue: string, text: string, actionSelect:string, actionEnter: string) {
    cy.get(`[${locatorName}="${locatorValue}"]`).type(text, { force: true }).type(actionSelect).type(actionEnter).debug();
    return this;
  }

  
//costum
  clickOnDomIndexChildIndex(locatorName: string, locatorValue: string, index : number, child : string, childIndex : number) {
    cy.get(`[${locatorName}="${locatorValue}"]`).eq(index).find(child).eq(childIndex).click();
    return this;
  }

  clickOnDomIndex(locatorName: string, locatorValue: string, index : number) {
    cy.get(`[${locatorName}="${locatorValue}"]`).eq(index).click();
    return this;
  }

  clickOnDom(dom: string) {
    cy.get(dom).click();
  }
